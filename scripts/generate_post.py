...
- "신청 방법"처럼 순서가 있는 절차를 설명할 때는 문단으로 풀어쓰지 말고 마크다운 순서
  목록(1. 2. 3.)으로 단계를 나눠 쓰세요 — 화면에 번호가 매겨진 단계로 시각적으로 표시됩니다.
...
sources는 절대 빈 배열로 두지 마세요. 본문에서 언급한 담당기관(예: 한국장학재단, 한국전력,
복지로, 고용노동부 등)의 공식 홈페이지를 최소 1개는 반드시 넣으세요. 정확한 URL을 모르면
기관명으로 신뢰할 수 있는 공식 도메인을 추정해서 넣으세요 (예: 한국장학재단 -> kosaf.go.kr).
"""
